...
export function AuthSplit({
  ...
  asideBody = "The AI reads the resume and surfaces fit. Shortlist and reject stay with you.",
    ...
    <div className="grid h-dvh w-full grid-cols-1 overflow-hidden lg:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
      <aside className="relative hidden min-h-0 min-w-0 overflow-hidden bg-brand text-white lg:flex lg:flex-col">
        ...
        <div className="relative shrink-0 px-10 pb-8 xl:px-14 xl:pb-10">
          <h2 className="max-w-[16ch] text-2xl font-bold leading-[1.15] xl:text-3xl">{asideHeadline}</h2>
          <p className="mt-3 max-w-[38ch] text-sm leading-6 text-white/70">{asideBody}</p>
          <p className="mt-4 text-xs text-white/45">{asideFootnote}</p>
        </div>
      </aside>

      <div className="flex min-h-0 min-w-0 flex-col overflow-hidden bg-surface">
        <div className="flex shrink-0 items-center px-5 py-3 sm:px-10 sm:py-4 lg:hidden">
          <Brand />
        </div>
        <div className="flex min-h-0 flex-1 flex-col justify-center overflow-hidden px-5 py-4 sm:px-10 sm:py-5 lg:py-6 xl:px-16">
          <div className="mx-auto w-full max-w-md">
            <h1 className="text-2xl font-bold">{title}</h1>
            <p className="mt-1.5 text-sm leading-5 text-ink-secondary">{description}</p>
